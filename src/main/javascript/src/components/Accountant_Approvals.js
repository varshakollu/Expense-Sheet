...
export class Accountant_Approvals extends React.Component {
  ...
  handleOpenModal(currentExpenseID, currentExpenseFirstName, currentExpenseLastName, currentExpenseCreationDate, currentExpenseName, currentExpenseAmount, currentManagerFirstName, currentManagerLastName, currentStatusInfo) {
    this.setState({
  ...
  render() {
    ...
    this.state.renderedUsers = this.state.statuses.filter(
      (p) => {
        if (this.state.searchValue) {
          return p.managerFirstName.toLowerCase().indexOf(this.state.searchValue) !== -1 || p.managerLastName.toLowerCase().indexOf(this.state.searchValue) !== -1 || p.firstName.toLowerCase().indexOf(this.state.searchValue) !== -1 || p.lastName.toLowerCase().indexOf(this.state.searchValue) !== -1 || p.expenseName.toLowerCase().indexOf(this.state.searchValue) !== -1 || p.statusInfo.toLowerCase().indexOf(this.state.searchValue) !== -1;
        }
        else {
          return p;
        }
              ...
              <tr scope="row">
                <td style={tableBorderStyle}>
                  <button
                    ...
                    onClick={() => { this.handleOpenModal(p.expenseID, p.firstName, p.lastName, p.creationDate, p.expenseName, p.amount, p.managerFirstName, p.managerLastName, p.statusInfo) }}>
    ...
    return (
      <div style={{ marginLeft: '17%' }}>
        <h3 style={{ marginBottom: '2%' }} >Expenses approved by Managers</h3>
...
`}</style>
        ...
        <div style={{ float: 'none', marginBottom: '2%' }}>
          <input id="searchField" style={{ width: '27%', display: 'inline-block' }} className="form-control" type="text" placeholder="Search Employee/Expense/Manager Name, Status" onChange={this.searchInputChange} aria-label="Search" />
          ...
          <div style={{ float: 'right', marginRight: '5%' }}>
            <ReactHTMLTable_ToExcel
              ...
              filename={"Expenses Approved By Managers"}
              sheet="Expenses Approved By Managers"
              buttonText="Download"
              img="image-xls"
        ...
        <div>
          ...
          >
            <div>
              ...
              <p><strong>Approving Manager : </strong>{this.state.currentManagerFirstName} {this.state.currentManagerLastName}</p>
